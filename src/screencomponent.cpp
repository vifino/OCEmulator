...
void ScreenComponent::draw()
{
    ...
    for (std::vector<std::vector<pixInfo> >::iterator it = pixels.begin(); it != pixels.end(); ++it)
    {
        ...
        for (std::vector<pixInfo>::iterator pt = it->begin(); pt != it->end(); ++pt)
        {
            ...
            //screen->renderText(xOn * pixW, yOn * pixH, 0, str.c_str());
            screen->ffont.Render("a", 1, FTPoint(xOn * pixW, yOn * pixH, 0), FTPoint(), FTGL::RENDER_FRONT);
            //screen->glyph->Render(FTPoint(xOn * pixW, yOn * pixH), FT_RENDER_MODE_NORMAL);
            yOn++;
        }
        xOn++;
    }

    std::cout << "Drew in " << m.elapsed() << std::endl;
}
